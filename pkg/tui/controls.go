...
func (m *Model) enabled(id string) bool {
	...
	case "clear":
		return len(f.Dates) > 0 || len(f.IncidentSeconds) > 0 || len(f.Guilds) > 0 || len(f.ExcludedGuilds) > 0 || len(f.ChannelTypes) > 0 || len(f.ExcludedChannelTypes) > 0 || f.From != "" || f.Until != "" || f.Search != "" || f.Media != "" || f.Mode != "auto" || f.DateBefore != 0 || f.DateAfter != 0 || f.Channel != "" || f.HideEventOnly || m.DayInput.Value() != "" || m.SearchInput.Value() != ""
	case "previous":
